...
(function () {
  ...
  function parseColumnDef(def, table) {
    ...
    if (!m) return;
    // DEFAULT value: number, quoted string, (expression) — read with balanced
    // parens, DEFAULT (CURDATE()) must not truncate — or bare literal/function
    let dflt = '';
    const dm = def.match(/\bDEFAULT\s+/i);
    if (dm) {
      const at = dm.index + dm[0].length;
      if (def[at] === '(') {
        const bal = readBalanced(def, at);
        if (bal) dflt = '(' + bal.body + ')';
      } else {
        const m2 = def.slice(at).match(/^(-?\d+(?:\.\d+)?|'(?:[^']|'')*'|"(?:[^"]|"")*"|[\w$]+(?:\s*\([^)]*\))?)/);
        if (m2) dflt = m2[1].trim();
      }
    }
